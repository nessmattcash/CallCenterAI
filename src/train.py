...
pipeline = Pipeline(
    [
        ...
        (
            "svm",
            LinearSVC(
                C=1.0, class_weight=class_weight_dict, random_state=42, max_iter=2000
            ),
        ),
    ]
)
...
calibrated_svc = CalibratedClassifierCV(
    LinearSVC(C=1.0, class_weight=class_weight_dict, random_state=42, max_iter=2000),
    cv=5,
    method="sigmoid",
)
...
for i, example in enumerate(examples):
    ...
    print(
        f"Example {i+1}: '{example}' -> Predicted: {predictions[i]}, Confidence: {confidence:.1f}%"
    )
